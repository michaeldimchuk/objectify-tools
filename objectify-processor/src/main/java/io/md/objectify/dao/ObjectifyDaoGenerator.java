...
class ObjectifyDaoGenerator {
  ...
  void generate(TypeElement entity) {
    ...
    try {
      JavaFileObject daoFile = filer.createSourceFile(getObjectifyDaoName(entity));
  ...
  private String getObjectifyDaoName(TypeElement entity) {
  ...
  private SourceFileBuilder getSourceFileBuilder(TypeElement entity, EntityDefinition definition) {
    ...
    return new SourceFileBuilder(getTemplate(definition));
  }
